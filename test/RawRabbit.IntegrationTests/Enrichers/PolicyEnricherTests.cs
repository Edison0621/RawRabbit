...
public class PolicyEnricherTests
{
	...
	public async Task Should_Use_Custom_Policy()
	{
		...
		AsyncFallbackPolicy defaultPolicy = Policy
			.Handle<Exception>()
			.FallbackAsync(ct =>
			{
				defaultCalled = true;
		...
		AsyncRetryPolicy declareQueuePolicy = Policy
			...
			.RetryAsync(async (e, retryCount, ctx) =>
			{
				...
				QueueDeclaration queue = new(defaultQueueCfg) { Name = ctx.GetQueueName(), Durable = false};
