...
namespace RawRabbit.DependencyInjection
{
	public class SimpleDependencyInjection : IDependencyRegister, IDependencyResolver
	{
		...
		{
			Lazy<TImplementation> lazy = new Lazy<TImplementation>(() => (TImplementation)instanceCreator(this));
		...
		{
			Lazy<TImplementation> lazy = new Lazy<TImplementation>(() => (TImplementation)this.CreateInstance(typeof(TImplementation), Enumerable.Empty<object>()));
			this.AddTransient<TService, TImplementation>(resolver => lazy.Value);
			return this;
		}
