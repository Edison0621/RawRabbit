...
public class ExclusiveLock : IExclusiveLock, IDisposable
{
	...
	{
		object theLock = this._lockDictionary.GetOrAdd(obj, _ => new object());
		SemaphoreSlim semaphore = this._semaphoreDictionary.GetOrAdd(theLock, _ => new SemaphoreSlim(1,1));
		return semaphore
			.WaitAsync(token)
			.ContinueWith(_ => theLock, token);
	}

	public Task ReleaseAsync(object obj)
	{
		SemaphoreSlim semaphore = this._semaphoreDictionary.GetOrAdd(obj, _ => new SemaphoreSlim(1, 1));
		semaphore.Release();
