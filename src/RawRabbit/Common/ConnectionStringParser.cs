...
{
	private static readonly Regex _mainRegex = new(@"((?<username>.*):(?<password>.*)@)?(?<hosts>[^\/\?:]*)(:(?<port>[^\/\?]*))?(?<vhost>\/[^\?]*)?(\?(?<parameters>.*))?");
	private static readonly Regex _parametersRegex = new(@"(?<name>[^?=&]+)=(?<value>[^&]*)?");
	...
	public static RawRabbitConfiguration Parse(string connectionString)
	{
		...
		RawRabbitConfiguration cfg = new()
		{
